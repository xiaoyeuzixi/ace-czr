...
def patch_bundle(source: Path, destination: Path) -> None:
    ...
    for node in bundle.nodes:
        ...
        while True:
            ...
            if cursor < 0:
                break
            matches.append((node.path, node.offset + cursor))
            cursor += 1

    if len(matches) != 1 or not matches[0][0].startswith("CAB-"):
        raise ValueError(f"expected one current-build anchor, got {matches}")

    node_path, absolute_offset = matches[0]
    ...
    if OLD_CONTEXT in rebuilt_data or rebuilt_data.count(NEW_CONTEXT) != 1:
