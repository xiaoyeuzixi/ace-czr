...
def patch_bundle(bundle: Bundle) -> list[tuple[str, int]]:
    ...
    for node in bundle.nodes:
        ...
        count = node_data.count(OLD_SEQUENCE)
        if count == 0:
            continue
        if count != 1:
            raise ValueError(f"node {node.path!r} has {count} patch anchors")
        relative_offset = node_data.index(OLD_SEQUENCE)
        absolute_offset = node.offset + relative_offset
        bundle.data[absolute_offset : absolute_offset + len(OLD_SEQUENCE)] = NEW_SEQUENCE
...
def verify_bundle(path: Path, expected: list[tuple[str, int]]) -> None:
    ...
    for node in bundle.nodes:
        node_data = bytes(bundle.data[node.offset : node.offset + node.size])
        if OLD_SEQUENCE in node_data:
            raise ValueError(f"old sequence remains in {node.path!r}")
        cursor = 0
        while True:
            cursor = node_data.find(NEW_SEQUENCE, cursor)
            if cursor < 0:
                break
